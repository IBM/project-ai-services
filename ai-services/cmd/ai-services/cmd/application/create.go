...
func downloadImagesForTemplate(runtime runtime.Runtime, templateName, appName string) error {
	...
	if skipImageDownload {
		// create a new imagePull object
		imagePull := image.NewImagePull(runtime, image.PullNever, appName, templateName)
		return imagePull.Run()
	}

	// create a new imagePull object based on imagePullPolicy flag

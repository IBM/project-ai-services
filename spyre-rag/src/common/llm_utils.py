...
def query_vllm(question, documents, llm_endpoint, llm_model, stop_words, max_new_tokens, temperature,
...
def query_vllm_non_stream(question, documents, llm_endpoint, llm_model, stop_words, max_new_tokens, temperature,
    ...
    headers, payload = query_vllm(question, documents, llm_endpoint, llm_model, stop_words, max_new_tokens, temperature, False,
                                  max_input_length=6000, dynamic_chunk_truncation=True)
    try:
...
def query_vllm_stream(question, documents, llm_endpoint, llm_model, stop_words, max_new_tokens, temperature,
                max_input_length=6000, dynamic_chunk_truncation=True):
    headers, payload = query_vllm(question, documents, llm_endpoint, llm_model, stop_words, max_new_tokens, temperature, True,
                                  max_input_length=6000, dynamic_chunk_truncation=True)
    try:
        # Use requests for synchronous HTTP requests
        logger.debug("STREAMING RESPONSE")

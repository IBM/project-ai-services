...
logger = get_logger("summarize_database")
...
def create_db_engine(echo: bool = False) -> Engine:
    ...
    logger.info(f"Summarize service database URL: {safe_url}")
    ...
    def receive_connect(dbapi_conn, connection_record):
        ...
        logger.debug("New database connection established for summarize service")
    
    @event.listens_for(engine, "close")
    def receive_close(dbapi_conn, connection_record):
        """Log closed database connections."""
        logger.debug("Database connection closed for summarize service")
...
try:
    ...
    logger.info("Summarize service database engine created successfully")
except ValueError as e:
    logger.warning(f"Summarize service database engine not initialized: {e}")
...
def check_db_connection() -> bool:
    ...
    """
    if not engine:
        logger.error("Summarize service database engine not initialized")
        return False
    
    try:
        with engine.connect() as conn:
            conn.execute(text("SELECT 1"))
        logger.info("Summarize service database connection check: OK")
        return True
    except Exception as e:
        logger.error(f"Summarize service database connection check failed: {e}")
        return False
...
def close_db_connections() -> None:
    ...
    if engine:
        engine.dispose()
        logger.info("Summarize service database connections closed")
